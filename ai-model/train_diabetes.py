"""
Train a TensorFlow model for diabetes prediction using the MediSphere Health Twin dataset.
"""
...
def build_diabetes_model(input_shape: int) -> tf.keras.Model:
    """Construct a Keras Sequential model for binary diabetes classification."""
...
def evaluate_model(model: tf.keras.Model, X_test: np.ndarray, y_test: np.ndarray) -> None:
    """Compute and print evaluation metrics for the diabetes model."""
...
def main() -> None:
    ...
    y_train = y_train_df["diabetes"].values
    y_test = y_test_df["diabetes"].values

    model = build_diabetes_model(X_train.shape[1])

    checkpoint_path = models_dir / "diabetes_model.keras"
    callbacks = [
